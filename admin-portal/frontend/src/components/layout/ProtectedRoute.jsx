...
export default function ProtectedRoute({ children }) {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <LoadingSpinner size="lg" message="Verifying administrative credentials..." />
      </div>
    );
  }
  ...
  // Exclusive role check: ADMIN only
  if (!isAdmin) {
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-xl border border-rose-200 p-6 shadow-md text-center">
          ...
          <p className="text-xs text-slate-600 mt-2 leading-relaxed">
            The BhumiPatra Administrator Portal is strictly restricted to accounts with the{' '}
            <span className="font-semibold text-rose-700">ADMIN</span> role.
          </p>
          <div className="my-4 p-3 bg-slate-50 rounded border border-slate-200 text-xs text-left">
            ...
            <p className="mt-1"><strong>Current Role:</strong> {user?.role || 'None'}</p>
          </div>
          <Button
            variant="danger"
            size="sm"
