...
export default function StatCard({
  ...
  // Never invent values: if value is null or undefined, display '—'
  let displayValue = '—';
  if (value !== undefined && value !== null) {
    if (typeof value === 'number') {
      displayValue = formatValue ? value.toLocaleString() : value;
    } else {
      displayValue = value;
    }
  }
  ...
  return (
    <div className={`bg-white rounded-xl border ${style.border} p-5 shadow-sm transition-all hover:shadow-card-hover`}>
