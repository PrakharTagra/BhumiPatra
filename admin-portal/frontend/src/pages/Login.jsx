...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Verify Administrator RBAC
      if (user && user.role && user.role !== 'ADMIN') {
        setErrorMessage('Access Denied: Only accounts with the ADMIN role are permitted to log in to this portal.');
        showError('Forbidden: Account lacks administrative privileges.');
        setLoading(false);
        return;
      }

      login(token, user || { email, role: 'ADMIN' });
      showSuccess('Administrative authentication successful.');
      
      const destination = location.state?.from?.pathname || '/';
      navigate(destination, { replace: true });
    } catch (err) {
      const msg = err.customMessage || err.response?.data?.message || 'Login failed. Please verify credentials.';
      setErrorMessage(msg);
      showError(msg);
    } finally {
      setLoading(false);
